...
module.exports = {
    addGenresInCharge: async function (req, res) {
    ...
    deleteGenresInCharge: async function (req, res) {
        const korisnikId = req.param('id');
        const zaduzeniZanroviId = req.body.zaduzeniZanroviId;
        ...
        const existingRelations = await ZaduzenZanr.find({ zaduzenKorisnik: korisnikId, zaduzenZanr: zaduzeniZanroviId });
        if (existingRelations.length !== zaduzeniZanroviId.length) {
            return res.status(400).json('Taj žanr nije zadužen za tog korisnika!');
        }

        await Korisnik.removeFromCollection(korisnikId, 'zaduzenZanr', zaduzeniZanroviId);

        return res.json('Korisniku su uspješno oduzeti zaduženi žanrovi!');
    },
